...
export default function CoursePage() {
  ...
  // Memoize loadCourse to allow its use in dependency arrays
  ...
  // Memoize loadFiles to allow its use in dependency arrays
  ...
  // Memoize filtering to prevent unnecessary re-calculations on render
  ...
  // Use functional update to avoid 'selectedFiles' dependency
  ...
  // Stable callback for downloading files
  const handleDownloadFile = useCallback((fileId: string) => {
    const fileToDownload = files.find(f => f.id === fileId)
    if (fileToDownload) {
      setToast({ isVisible: true, message: `Downloading ${fileToDownload.name}`, type: 'info' })
  ...
  // Stable callback for deleting files using functional update
  const handleDeleteFile = useCallback((fileId: string) => {
    deleteFile(fileId)
    setFiles(prev => prev.filter(f => f.id !== fileId))
    loadCourse()
    setToast({ isVisible: true, message: 'File deleted successfully', type: 'success' })
  }, [loadCourse])
